    ...
    for original_contig_id, part_ids_d in all_originals.items():
        if len(part_ids_d) > 1:
            ...
            if len(c.values()) > 1:
                ...
                # Pick a random cluster from the tied ones
                most_common_count = c.most_common(1)[0][1]  # Get the highest frequency count
                tied_clusters = [a for a, b in c.items() if b == most_common_count]
                cluster_id = random.choice(tied_clusters)
        else:
