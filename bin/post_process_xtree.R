...
tax_ed <- tax.tm.ra
dfx<-tax_ed
tax_ed$data<-row.names(tax_ed)
col_numb<-ncol(tax_ed)-1
df_data<-as.data.frame(tax_ed[,ncol(tax_ed)])
df1<-read.table(text = df_data[[1]], fill = TRUE, sep = ";")

tax<-c("class", "family", "genus", "order", "phylum", "species", "domain")
tax_names<-c("c","f","g","o","p","s","d")

taxon<-data.frame(tax_names, tax)

xx<-apply(df1, 2, function(x) {strsplit(x, "__")[[1]][1]})
colnames(df1)<-taxon$tax[match(xx, taxon$tax_names)]
df2<-cbind(df1, dfx)
rownames(df2)<-NULL

write.table(df2,paste(args[[5]],'/',args[[6]],'_rax.tsv',sep=''),F,F,'\t', row.names = FALSE)
